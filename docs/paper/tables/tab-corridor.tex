\begin{table}[t]
\caption{Distance to the nearest \textit{other} cable, real route vs.\ its
placebo-displaced copy, under progressively stricter controls. The first three
rows exclude candidates by identity; the rest remove the landfall geometry that
identity exclusion cannot touch. The bold row is the figure this paper claims.}
\label{tab:corridor}
\centering\footnotesize
\begin{tabular}{@{}lrrrrr@{}}
\toprule
Control & $n$ & Real & Placebo & Closer & $p$ \\
 & & (km) & (km) & on & \\
\midrule
Any other route & 336 & 2.1 & 9.8 & 90\% & $<$\,$10^{-4}$ \\
Excl.\ same named cable & 336 & 2.2 & 9.9 & 90\% & $<$\,$10^{-4}$ \\
Excl.\ same agency & 301 & 7.1 & 22.2 & 83\% & $<$\,$10^{-4}$ \\
\midrule
\quad $+$ trim 10\,km & 227 & 9.3 & 22.7 & 79\% & $<$\,$10^{-4}$ \\
\quad $+$ trim 30\,km & 183 & 11.6 & 23.6 & 78\% & $<$\,$10^{-4}$ \\
\quad $+$ trim 50\,km & 158 & 11.5 & 22.3 & 79\% & $<$\,$10^{-4}$ \\
\midrule
\textbf{\quad $+$ trim 30, excl.\ shared landing} & \textbf{182} & \textbf{19.0} & \textbf{26.6} & \textbf{74\%} & \textbf{$<$\,$10^{-4}$} \\
\bottomrule
\end{tabular}
\end{table}
