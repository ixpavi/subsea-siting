...
% Let long URLs break at any letter, as IEEE's own reference lists do, instead
% of stretching the words before them across the column.
\makeatletter
...
\input{numbers}
\hypersetup{
  pdftitle={Corridor Reuse Outperforms Bathymetry in Submarine Cable Route
    Prediction: A Controlled Evaluation on \nCorpusRoutes{} As-Laid Routes},
  pdfauthor={Pavitra Sharma and Dhanush D},
  pdfkeywords={submarine cables, route planning, least-cost path, model
    validation, placebo control, bathymetry}}

\newcommand*{\authoraffiliation}{the Department of Computing Technologies,
...
\author{Pavitra~Sharma and Dhanush~D%
\thanks{\ifx\authoraffiliation\empty\else Pavitra Sharma and Dhanush D are
with \authoraffiliation.\ \fi
Corresponding author: Pavitra Sharma\hfil\break
(e-mail: ps6758@srmist.edu.in).}%
\thanks{Analysis code and the full reproduction pipeline are available at
\url{https://github.com/ixpavi/subsea-siting}.}}

\markboth{}%
{Sharma \MakeLowercase{\textit{et al.}}: Corridor Reuse Outperforms Bathymetry
in Submarine Cable Route Prediction}
...
2{,}000\,km ($p=\nLongHaulP$). Its operating range is explained by a per-route
rule (Spearman $\rho=\nErRho$, $n=\nErN$) that shows when it will fail.
...
laid~\cite{makrakis2023gis}, and a 2026 study of offshore-wind export cables
generates least-cost routes from weighted criteria, treating existing cables
only as exclusion zones, without comparing its routes with installed
ones~\cite{walsh2026owf}. The patent trains a model on existing routes
...
\item \textbf{A rule that explains its failures.} Corridor following helps
...
cable siting with seismic fault avoidance~\cite{makrakis2023gis}. The
optimisation itself is patented: US~10,425,280~B2~\cite{cityu_patent_2019}
claims routing a new link to an existing network over a triangulated terrain
surface, trading construction cost against the expected number of repairs.
...
The corpus is Europe-weighted but not shelf-limited (Fig.~\ref{fig:corpus}):
the French SHOM layer covers overseas territories, reaching the Pacific, Indian
Ocean and Caribbean.
...
\input{tables/tab-corpus}

\begin{figure*}[t]
\centering
\includegraphics[width=\textwidth]{fig6-corpus-map.pdf}
\caption{Geographic coverage of the corpus, coloured by publishing agency.
Most of corpus length is northwest Europe and the Mediterranean, and
the trans-oceanic reach comes almost entirely from one agency's overseas
territories --- the concentration Section~\ref{sec:scope} reports, shown rather
than asserted. It is also why Section~\ref{sec:dose}'s ruggedness gradient is
confounded: in this corpus, agency and sea are not separable.}
\label{fig:corpus}
\end{figure*}
...
scored by the mean distance from the cable actually laid
(Fig.~\ref{fig:protocol}). Every terrain-aware
...
sea and a survey convention; a fold is only run where at least 15 routes
remain.

\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{fig7-protocol.pdf}
\caption{The evaluation protocol. Every method receives only the two endpoints
and is scored on distance from the cable actually laid. All terrain-aware
methods share one search, one grid and one endpoint treatment, so a difference
between them is attributable to the cost weights rather than to the machinery.}
\label{fig:protocol}
\end{figure}
...
\input{tables/tab-prediction}
\input{tables/tab-paired}
...
error falls from \nRegSeapath\,km to \nRegFitted\,km---a 27\% reduction---on
...
52\%, $p=0.58$). Whatever the terrain signal is, it is coarse enough that a
...
\input{tables/tab-corridor}

\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{fig8-control-ladder.pdf}
\caption{The corridor effect as each confound is removed. Identity exclusion
alone reports \nCorridorRaw\%; trimming the landfall approaches and then
excluding cables that share a named landing point brings it to
\nCorridorCtl\%. The first row is the figure a paper that stopped at identity
exclusion would have published.}
\label{fig:ladder}
\end{figure}
...
\input{tables/tab-bands}
\input{tables/tab-bandpaired}
...
useful result here for anyone building such a router because it shows which
neighbour data the method needs in order to help. It is descriptive, not causal: the ratio and the
...
\emph{Geographic coverage.} Most of corpus length is NW Europe and the
...
Section~\ref{sec:ratio} shows which neighbour data it needs in order to help.
